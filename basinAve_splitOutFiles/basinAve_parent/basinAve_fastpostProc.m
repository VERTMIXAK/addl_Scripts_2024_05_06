...
sfos.myGridFile = ['../../../InputFiles/Gridpak_parent/NGnest_100m_parent.nc'];

[dum1,dum2]=unix('pwd | rev |cut -d "/" -f2 | rev | cut -d "_" -f1');
sfos.myPrefix = lower(dum2(1:end-1));

HISfiles  = dir('../netcdfOutput_split/HIS_*');
HIS2files = dir('../netcdfOutput_split/HIS2_*');
...
for tt=1:Nt
    ...
    [dum1,dum2] = roms_get_date(['../netcdfOutput_split/',HIS2files(tt).name]);
    ...
    SST_rho    = sq(nc_varget(['../netcdfOutput_split/',HIS2files(tt).name],'temp_sur',[0,0,0],[1,-1,-1]) )   .* sfos.grid.mask_rho;
    SSS_rho    = sq(nc_varget(['../netcdfOutput_split/',HIS2files(tt).name],'salt_sur',[0,0,0],[1,-1,-1]) )   .* sfos.grid.mask_rho;
...
for tt=1:Nt
    ...
    SSH_rho    = sq(nc_varget(['../netcdfOutput_split/',HISfiles(tt).name],'zeta',[0,0,0],[1,-1,-1]) )    .* sfos.grid.mask_rho;
