function [AA,iters,MM] = rdmds(fnamearg,varname,varargin)
% [AA,iters,MM] = rdmds(fnamearg,varname,varargin)
% RDMDS  Read MITgcmUV meta/data files
%
% A = RDMDS(rootdir,varname)
% A = RDMDS(rootdir,varname,ITER)
% A = RDMDS(rootdir,varname,[ITER1 ITER2 ...])
% A = RDMDS(rootdir,varname,NaN)
% A = RDMDS(rootdir,varname,Inf)
% [A,ITS,M] = RDMDS(rootdir,varname,[...])
% A = RDMDS(rootdir,varname,[...],'rec',RECNUM)
%
%   A = RDMDS(rootdir,varname) reads data described by meta/data file format.
...
%
%
%   A = RDMDS(rootdir,varname,ITER) reads data described by meta/data file format.
%   varname is a string containing the "head" of the file name excluding the
...
%
%
%   A = RDMDS(rootdir,varname,[...],'rec',RECNUM) reads individual records from
...
%
%
%   A = RDMDS(rootdir,varname,ITER,MACHINEFORMAT) allows the machine format to be
%   A = RDMDS(rootdir,varname,MACHINEFORMAT)
...
for iter=1:size(iters,2);
  if iters(iter)>=0
    fname=sprintf('%s.%10.10i',varname,iters(iter));
  else
    fname=varname;   
  end
...
%  keyboard
  ...
  num=0;
  for i=1:length(dd);
    sprintf('%s/%s/%s*.meta',Dir,dd(i).name,fname)
    ddd= dir( sprintf('%s/%s/%s*.meta',Dir,dd(i).name,fname));
    for j=1:length(ddd);
      num=num+1;
      allfiles(num).name=[sprintf('%s/%s/',Dir,dd(i).name) ddd(j).name];
    end;
  end;
  
  if size(allfiles,1)==0
    disp(sprintf('No files match the search: %s%s*.meta',Dir,fname));
  ...
  for j=1:length(allfiles)
    ...
    [A,N,M,mG] = localrdmds([allfiles(j).name],ieee,recnum);
