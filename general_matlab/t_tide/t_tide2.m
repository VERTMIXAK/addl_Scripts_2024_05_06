...
ngood=length(gd);
%jgpComment:fprintf('   Points used: %d of %d\n',ngood,nobs)
...
%jgpComment:  fprintf('   percent of var residual after lsqfit/var original: %5.2f %%\n',100*(varxr/varx));  
else               % Complex time series
  varx=cov(real(xin(gd)));varxp=cov(real(xout(gd)));varxr=cov(real(xres(gd)));
%jgpComment:  fprintf('   percent of X var residual after lsqfit/var original: %5.2f %%\n',100*(varxr/varx));

  vary=cov(imag(xin(gd)));varyp=cov(imag(xout(gd)));varyr=cov(imag(xres(gd)));
%jgpComment:  fprintf('   percent of Y var residual after lsqfit/var original: %5.2f %%\n',100*(varyr/vary));
end;
...
if ii,
%jgpComment:  fprintf('   Do inference corrections\n');
...
%jgpComment:  fprintf('   Using nonlinear bootstrapped error estimates\n');
...
%jgpComment:  fprintf('   Using linearized error estimates\n');
  %
...
%jgpComment:   fprintf('   Generating prediction with nodal corrections, SNR is %f\n',synth);
   xout=t_predic(stime+[0:nobs-1]*dt/24.0,nameu,fu,tidecon,'lat',lat,'synth',synth,'anal',ltype);
 elseif ~isempty(stime), 
%jgpComment:   fprintf('   Generating prediction without nodal corrections, SNR is %f\n',synth);
...
%jgpComment: fprintf('   Returning fitted prediction\n');
end;
...
if isreal(xin),    % Real time series
  varx=cov(xin(gd));varxp=cov(xout(gd));varxr=cov(xres(gd));
%jgpComment:  fprintf('   percent of var residual after synthesis/var original: %5.2f %%\n',100*(varxr/varx));  
else               % Complex time series
  varx=cov(real(xin(gd)));varxp=cov(real(xout(gd)));varxr=cov(real(xres(gd)));
%jgpComment:  fprintf('   percent of X var residual after synthesis/var original: %5.2f %%\n',100*(varxr/varx));

  vary=cov(imag(xin(gd)));varyp=cov(imag(xout(gd)));varyr=cov(imag(xres(gd)));
%jgpComment:  fprintf('   percent of Y var residual after synthesis/var original: %5.2f %%\n',100*(varyr/vary));
end;
...
if fid>1,
%jgpComment: fprintf(fid,'\n%s\n',['file name: ',filen]);
elseif fid==1,
%jgpComment: fprintf(fid,'-----------------------------------\n');
end

if fid>0,
%jgpComment:  fprintf(fid,'date: %s\n',date);
%jgpComment:  fprintf(fid,'nobs = %d,  ngood = %d,  record length (days) = %.2f\n',nobs,ngood,length(xin)*dt/24);
%jgpComment:  if ~isempty(stime); fprintf(fid,'%s\n',['start time: ',datestr(stime)]); end
%jgpComment:  fprintf(fid,'rayleigh criterion = %.1f\n',ray);
%jgpComment:  fprintf(fid,'%s\n',nodcor);
...
%jgpComment:  fprintf(fid,'percent var predicted/var original= %.1f %%\n',100*varxp/varx);

  if isreal(xin)
%jgpComment:    fprintf(fid,'\n     tidal amplitude and phase with 95%% CI estimates\n');
%jgpComment:    fprintf(fid,'\ntide   freq       amp     amp_err    pha    pha_err     snr\n');
    for k=1:length(fu);
%jgpComment:      if snr(k)>synth, fprintf(fid,'*'); else fprintf(fid,' '); end;
%jgpComment:      fprintf(fid,'%s %9.7f %9.4f %8.3f %8.2f %8.2f %8.2g\n',nameu(k,:),fu(k),tidecon(k,:),snr(k));
    end
  else
%jgpComment:    fprintf(fid,'\ny0= %.3g, x trend= %.3g\n',imag(z0),imag(dz0));
...
%jgpComment:      fprintf(fid,'%s %9.7f %6.3f %7.3f %7.3f %6.2f %8.2f %6.2f %8.2f %6.2f %6.2g\n',...
%jgpComment:	nameu(k,:),fu(k),tidecon(k,:),snr(k));
    end
%jgpComment:    fprintf(fid,['\ntotal var= ',num2str(varx+vary),'   pred var= ',num2str(varxp+varyp) '\n']);
%jgpComment:    fprintf(fid,'percent total var predicted/var original= %.1f %%\n\n',100*(varxp+varyp)/(varx+vary));
  end
...
%jgpComment:disp(['   number of standard constituents used: ',int2str(length(ju))])
  ...
  for k=1:size(infname,1),
   ...
   if isempty(j1),
    ...
    if isempty(j1),
      ...
      jref(k)=j1;
%jgpComment:      fprintf(['   Inference of ' namei(k,:) ' using ' nameu(j1,:) '\n']);
    end;
   end;
  end;    
  jinf(isnan(jref))=NaN;
