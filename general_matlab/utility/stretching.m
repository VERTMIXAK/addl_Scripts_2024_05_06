...
% svn $Id: stretching.m 647 2013-01-22 23:40:00Z arango $
%=========================================================================%
...
% ************  
% JGP modify to accomodate UH stretching  

elseif (Vstretching == 5),
    
  c1=1.0; c2=2.0; p5=0.5;
    
  if (kgrid == 1),     % this is for w_grid
    s = (0:N);
    s = -(s.^2 - 2*N*s + s + N^2 - N) / ...
        (N^2 - N) - .01*(s.^2 - N*s)/(1-N);
  else                 % this is for rho_grid
    s = (1:N)-.5;
    s = -(s.^2 - 2*N*s + s + N^2 - N) / ...
        (N^2 - N) - .01*(s.^2 - N*s)/(1-N);
  end
  ...
  if (theta_s > 0),
    Csur=(c1 - cosh(theta_s * s) ) ./ (cosh(theta_s) - c1);
  else
    Csur=-s.^2;
  end
  if (theta_b > 0),
    Cbot=( exp(theta_b * (Csur + c1) ) - c1 ) / ( exp(theta_b) - c1) - c1;
    C=Cbot;
  else
    C=Csur;
  end  
